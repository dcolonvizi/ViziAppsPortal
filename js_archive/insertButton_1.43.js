        ...
        function initDialog() {
            ...
            if (ArgList[0].length > 0) {
                ...
                if (ArgList[0].length > 0) {
                    ...
                    insertButtonID.value = "Update Button";
                }

                var label = document.getElementById("label");
                label.value = ArgList[2];

                var view_response_radios = document.getElementsByName("view_response");

                var parse = ArgList[1].split(';');
                        ...
                        case 'share':
                         ...
                         case 'login_to_mcommerce':
                          ...
                          case 'void_charge':
                              ...
                              break;
                          case 'call_intuit_gopayment':
                              setComboValue(actions, 'call_intuit_gopayment');
                              var sub_parts = parts[1].split(',');
                              for (var j = 0; j < sub_parts.length; j++) {
                                  var share_parts = sub_parts[j].split('~');
                                  switch (share_parts[0]) {
                                      case 'amount_field':
                                          var intuit_gopayment_amount_field = document.getElementById("intuit_gopayment_amount_field");
                                          intuit_gopayment_amount_field.value = share_parts[1];
                                          break;
                                      case 'gopayment_response_field':
                                          var intuit_gopayment_response_field = document.getElementById("intuit_gopayment_response_field");
                                          intuit_gopayment_response_field.value = share_parts[1];
                                          break;
                                  }
                               }
                              break;                              
                ...
                ButtonImage.src = ArgList[4];
                init_default_button_image = ArgList[4];
            }
            else {
                var ButtonImage = document.getElementById("ButtonImage");
                var DefaultButtonImage = parent.document.getElementById("DefaultButtonImage");
                ButtonImage.src = DefaultButtonImage.value;
            }
            style = ArgList[3];
            getStyle();
        }

        var init_default_button_image;
        var error = false;
        function insertButton() //fires when the Insert Link button is clicked
        {
            ...
            if (view == 'next_page_view') {
                ...
                ArgList[1] = 'next_page:page~' + next_page_value.replaceAll(" ", "_") + ";";
            }
                ...
                ArgList[1] = 'if_then_next_page:next_page_condition_value~' + next_page_condition_value.replaceAll("<", "#lt").replaceAll(">", "#gt").replaceAll("=", "#eq") + ',page_if_true_value~' + page_if_true_value.replaceAll(" ", "_") + ',page_if_false_value~' + page_if_false_value.replaceAll(" ", "_") + ";";
            }
            else if (view == 'previous_page_view') {
                ArgList[1] = 'previous_page;';
            }
                ...
                if (!IsValidObjectName(phone.value)) {
                    alert('Phone field can only contain either a letter, number or "_" and be 1 to 100 characters long');
                    return;
                }
                ArgList[1] = 'call:phone_field~' + phone.value.replaceAll(" ", "_") + ";";
            }
                ...
                var sms_phone = document.getElementById("sms_phone_field");
                var to_email = document.getElementById("to_email_field");
                ArgList[1] = 'share:subject_field~' + subject.value.replaceAll(" ", "_") +
                ...
                ArgList[1] = 'email:to_email_field~' + to_email_field2.value.replaceAll(" ", "_") +
                ...
                ArgList[1] = 'sms:sms_phone_field~' + sms_phone_field2.value.replaceAll(" ", "_") +
                ...
                if (!IsValidObjectName(image_field.value)) {
                    alert('Photo image field can only contain either a letter, number or "_" and be 1 to 100 characters long');
                    return;
                }
                var compression = document.getElementById("compression");
                ...
                ArgList[1] += 'image_field~' + image_field.value.replaceAll(" ", "_") + ',compression~' + compression.value;
                ...
                varicon_attributes = '';
                if (icon_field.value != '')
                    ArgList[1] += ",icon_field~" + icon_field.value.replaceAll(" ", "_") + ",icon_width~" + icon_width.value + ",icon_height~" + icon_height.value;

                ArgList[1] += ';';
            }
            else if (view == 'capture_signature_view') {
                var signature_image_field = document.getElementById("signature_image_field");
                ArgList[1] = 'capture_signature:signature_image_field~' + signature_image_field.value.replaceAll(" ", "_") + ";";
            }

            else if (view == 'capture_doc_view') {
                var doc_case_field = document.getElementById("doc_case_field");
                ArgList[1] = 'capture_doc:doc_case_field~' + doc_case_field.value + ';';
            }
            else if (view == 'login_to_mcommerce_view') {
                ArgList[1] = 'login_to_mcommerce:';
                error = false;
                $('.mcommerce').each(function (index, element) {
                    ...
                    if (index > 0)
                        ArgList[1] += ',';
                    ArgList[1] += element.id + '~' + element.value;
                });
                if (error)
                    return;
                ArgList[1] += ';';
                ...
                ArgList[1] = 'manual_card_charge:;';
            }
            else if (view == 'void_charge_view') {
                ArgList[1] = 'void_charge:';
                error = false;
                $('.param').each(function (index, element) {
                    if (element.value.length == 0) {
                        alert('All parameters must have values.');
            ...
            else if (view == 'call_intuit_gopayment_view') {
                var intuit_gopayment_amount_field = document.getElementById("intuit_gopayment_amount_field");
                if (intuit_gopayment_amount_field.value.length == 0) {
                    alert('The name of the amount field must be set.');
                    return;
                }
                if (!IsValidObjectName(intuit_gopayment_amount_field.value)) {
                    alert('Amount field can only contain either a letter, number or "_" and be 1 to 100 characters long');
                    return;
                }
                var intuit_gopayment_response_field = document.getElementById("intuit_gopayment_response_field");
                if (intuit_gopayment_response_field.value.length == 0) {
                    alert('The name of the response field must be set.');
                    return;
                }
                if (!IsValidObjectName(intuit_gopayment_response_field.value)) {
                    alert('Response field can only contain either a letter, number or "_" and be 1 to 100 characters long');
                    return;
                }
                ArgList[1] = 'call_intuit_gopayment:amount_field~' + intuit_gopayment_amount_field.value.replaceAll(" ", "_") +
                             ',gopayment_response_field~' + intuit_gopayment_response_field.value.replaceAll(" ", "_") + ';';
            }
            else if (view == 'capture_process_document_view') {
                ArgList[1] = 'capture_process_document:';
                error = false;
                $('.param').each(function (index, element) {
                    if (element.value.length == 0) {
                        alert('All parameters must have values.');
                    ...
                    if (index > 0)
                        ArgList[1] += ',';
                    ArgList[1] += element.id + '~' + element.value.replaceAll(':', '%3A');
                });
                if (error)
                    return;
                ArgList[1] += ';';
                ...
                ArgList[1] = 'manage_document_case:';
                error = false;
                $('.param').each(function (index, element) {
                    if (element.value.length == 0) {
                        alert('All parameters must have values.');
                    ...
                    if (element.value.indexOf('<') >= 0 || element.value.indexOf('\n') >= 0)
                        ArgList[1] += element.id + '~' + escape(element.value);
                    else
                        ArgList[1] += element.id + '~' + element.value.replaceAll(':','%3A');
                });
                if (error)
            ...
            if (docompute.checked) {
                ...
                ArgList[1] += 'compute:' + compute.value.replaceAll("=", "~").replaceAll(";", "|").replaceAll(">", "#gt").replaceAll("<", "#lt") + '|'; // '=' is a special character, '~' is substituting for it
            }

  
            var label = document.getElementById("label");
            if (label.value == null || label.value == '') {
                alert('Button Label must be filled');
                return;
            }
 
            ArgList[2] = label.value;
            ArgList[3] = style;
            var ButtonImage = document.getElementById("ButtonImage");
            ArgList[4] = ButtonImage.src;

            //set default button image
            var DefaultButtonImage = parent.document.getElementById("DefaultButtonImage");
            DefaultButtonImage.value = ButtonImage.src;

            parent.window.InsertButtonCallback(ArgList);
        }
        function onSetButtonColorClientClose(sender, eventArgs) {
        ...
        function showSetButtonColorClient() {
            var oWin = radopen('../Dialogs/Design/ButtonColor.aspx', 'buttonColor');
            oWin.set_visibleTitlebar(true);
            oWin.set_visibleStatusbar(false);
            oWin.set_modal(true);
            oWin.setSize(200, 460);
            oWin.moveTo(450, 0);
            oWin.add_close(onSetButtonColorClientClose);
            return false;
        }
